...
import com.example.free.Classes.FFT;
import com.example.free.Classes.HandleData;
import com.example.free.Classes.LandEachButton;
import com.example.free.Mp3Handle.Mp3FileReader;
...
public class FourGameActivity extends BaseActivity {
    ...
    final Context context=FourGameActivity.this;
    ...
    Button[] bottoms=new Button[4];

    static ConstraintLayout buttonConstraintLayout;//按钮放置的视图
    public static ArrayList<ArrayList<LandEachButton>> buttons=new ArrayList();//按钮滑块们
    ...
    int buttonWidth=90;//dp
    int bottomHeight=60;//底部按钮的高度，也是最后决定good best等级的高度
    int buttonHeight=45;
    int[] X={60,160,260,360};//轨道 //button 宽80dp  bottom 120dp
    int startY=0;
    int marginTop=250;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //这里要重新初始化活动状态，不然第二次选歌后这里默认false，LandEachButton那里无法访问resultActivity
        activityState=true;
...
//                }
        ...
        timer.schedule(task, duration*(marginTop+buttonHeight)/(endY));
        //mediaPlayer.start();
...
//    }
    ...
    public void initButtons(){
        buttons.add(new ArrayList<LandEachButton>());//此处需要初始化！！！
        buttons.add(new ArrayList<LandEachButton>());
        buttons.add(new ArrayList<LandEachButton>());
        buttons.add(new ArrayList<LandEachButton>());
        int n=allTime.size();
        for(int i=0;i<n;i++) {
            ...
            int x=X[p];
            LandEachButton eachButton = new LandEachButton(getApplicationContext(),
    ...
    public void initBottom(){
        bottoms[0]=findViewById(R.id.land_bottom1);
        bottoms[1]=findViewById(R.id.land_bottom2);
        bottoms[2]=findViewById(R.id.land_bottom3);
        bottoms[3]=findViewById(R.id.land_bottom4);
        ...
        // }
        bottoms[2].setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if(buttons.get(2).size()!=0)
                    initToasts(2);
            }
        });
        bottoms[3].setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if(buttons.get(3).size()!=0)
                    initToasts(3);
            }
        });
...
//            Toast.makeText(context,buttons.get(i).size()+"",Toast.LENGTH_SHORT).show();
        ...
        View toastView =LayoutInflater.from(context).inflate(R.layout.toast_land, null);//得到toast样式
...
//        int d=(int)(bottoms[i].getY()-button.getY());
        ...
        if(buttons.get(0).size()+buttons.get(1).size()+buttons.get(2).size()+buttons.get(3).size()==0){
            final Intent iintent=new Intent(FourGameActivity.this,ResultActivity.class);
    ...
    void buttonsPause(){
        ...
        nowTime+=(int)(new Date().getTime()-dateHelp.getTime());
        if(buttons.get(0).size()+buttons.get(1).size()+buttons.get(2).size()+buttons.get(3).size()!=0){
            for(int i=0;i<4;i++){
